# CLAUDE.md — boatrace プロジェクト引き継ぎメモ

## このプロジェクトでの応対方針
- **常に日本語のみで返答する。**
- **思考過程（reasoning / 内部検討）は表示しない。** 結論と必要な説明だけを簡潔に返す。

---

## 目的
競艇（ボートレース）の公開データを毎日自動収集し、GitHub Pages（`docs/`）で閲覧できる
データポータルとして公開するプロジェクト。あわせて、朝に出す「見立て（荒れ/堅め判定）」を
夜の結果と突き合わせて検証する仕組みを持つ。

**方針上の重要な制約:** 公開ページ・見どころ生成では
**買い目・確率・勝者の断定・選手の内心推測は出さない**（`tenkai_logic.json` の方針に準拠）。
勝敗の主観的予想ではなく「水面傾向・展開の見どころ」と「荒れやすさの可視化」に留める。
判定や主役艇などの内部値は公開 `highlights.json` には入れず、非公開の `predictions/` にのみ残す。

---

## ディレクトリ構成
```
.github/workflows/   GitHub Actions（自動収集・検証のスケジュール定義）
scripts/             データ収集・集計・生成の Python スクリプト（＋HTMLテンプレート）
docs/                GitHub Pages で公開する静的サイト＋データ本体
  data/              風予報・荒れ指数・各種ロジック定数（weather.json / arare.json ほか）
  racers/            出走表（racers_today.csv）とビューワー
  motor/             モーター成績（motors_all.csv）とビューワー
  motor/, players/, payouts/, highlights/, stadium/ …各テーマのページとJSON/CSV
predictions/         朝の見立て（YYYYMMDD.json、非公開の検証用スナップショット）
results/             夜に生成する結果データ（未コミット＝生成物）
```

---

## スクリプトの役割（`scripts/`）
| スクリプト | 役割 | 主な入力 → 出力 |
|---|---|---|
| `scrape_racers.py` | 出走表スクレイプ（boatrace.jp） | 公式 → `docs/racers/racers_today.csv`, `index.html` |
| `scrape_motors.py` | モーター成績スクレイプ（boatrace.jp） | 公式 → `docs/motor/motors_all.csv`, `index.html` |
| `fetch_weather.py` | 24場の風予報取得（Open-Meteo） | API → `docs/data/weather.json` |
| `scrapeKimarite.py` | 選手別 決まり手率・前づけ傾向を直近183日集計（mbrace LZH） | 公式K配布 → `docs/players/racerKimarite.csv` |
| `build_highlights.py` | 出走表＋モーター＋決まり手＋風から「見どころ/展開文」を生成 | 上記CSV/JSON → `docs/highlights/highlights.json`（＋非公開 `predictions/YYYYMMDD.json`） |
| `build_arare.py` | 荒れ指数（荒れ条件が何個揃うか）を可視化。標準ライブラリのみ | 出走表/モーター/風＋場定数 → `docs/data/arare.json` |
| `scrapeTokuyamaPayouts.py` | 徳山(18)の3連単払戻を収集（mbrace LZH） | 公式K配布 → `docs/payouts/tokuyamaPayouts.csv` |
| `buildTokuyamaManRate.py` | 徳山 払戻CSV → R別万舟率JSON集計 | CSV → `docs/payouts/tokuyamaManRate.json` |
| `scrapeKiryuPayouts.py` | 桐生(01)の3連単払戻を収集（mbrace LZH） | 公式K配布 → `docs/payouts/kiryuPayouts.csv` |
| `buildKiryuManRate.py` | 桐生 払戻CSV → R別万舟率JSON集計 | CSV → `docs/payouts/kiryuManRate.json` |
| `buildResults.py` | 当日全24場の着順(3連単)と配当を抽出（mbrace LZH） | 公式K配布 → `results/YYYYMMDD.json` |
| `verifyPredictions.py` | 朝の見立てと夜の結果を場×レースで突合し追記 | `predictions/` + `results/` → `verify_log.csv` |
| `build_verify_summary.py` | 検証ログの要約JSON生成（workflowから呼ばれる） | `verify_log.csv` → `docs/data/verify_summary.json` |
| `batch_update_nicknames.py` | 選手あだ名を手動一括更新（コピペで追記して実行） | `docs/players/profile.json` を書き換え |

補足:
- mbrace 公式配布は LZH 形式。解凍は Actions 上で `lhasa`（`lha` コマンド）または `lhafile`(pip) を使う。
- `verifyPredictions.py` の鉄則: **`predictions/` は結果を見る前の値。検証時に読むだけで一切書き換えない。**
- `build_verify_summary.py` は workflow から参照されるが、現状 git 未追跡（要確認）。

---

## GitHub Actions ワークフロー（`.github/workflows/`）
すべて `github-actions[bot]` が生成物をコミット＆プッシュする。時刻は UTC 指定（JST=UTC+9）。
それぞれ専用の `concurrency` グループでプッシュ競合を回避。

| ファイル | 名前 | トリガー | 実行内容 |
|---|---|---|---|
| `update_racers.yml` | 出走表データ自動更新 | 朝（当日確定）＋夜（翌日分、遅延対策で複数本） | `scrape_racers.py` |
| `update_motors.yml` | モーターデータ自動更新 | 朝＋夜（翌日予習） | `scrape_motors.py` |
| `update_weather.yml` | 気象データ自動更新 | 3時間ごと | `fetch_weather.py` |
| `update_highlights.yml` | 見どころJSON自動生成 | 「出走表データ自動更新」完了を受けて（`workflow_run`） | `build_highlights.py` |
| `update arare.yml` | 荒れ指数 更新 | 毎日 JST 8:45（出走表更新の後） | `build_arare.py` |
| `updateKimarite.yml` | update kimarite | 月2回（1日/15日） | `scrapeKimarite.py`（183日窓） |
| `updateResults.yml` | 見立て結果照合（夜） | 毎日 JST 23:37（全場終了後） | `buildResults.py` → `verifyPredictions.py` → `build_verify_summary.py` |
| `updateTokuyamaPayouts.yml` | update tokuyama payouts | 毎日（JST 翌9:20/10:20 相当） | `scrapeTokuyamaPayouts.py` → `buildTokuyamaManRate.py` |
| `updateKiryuPayouts.yml` | update kiryu payouts | 毎日 JST 翌0:20（結果確定後） | `scrapeKiryuPayouts.py` → `buildKiryuManRate.py` |

いずれも `workflow_dispatch` で手動実行可。払戻系は `YM`/`ym` 入力で対象月を指定できる。

---

## データの流れ
### 1) 日次の公開データ更新（朝〜日中）
```
公式(boatrace.jp) ─ scrape_racers.py ─→ docs/racers/racers_today.csv
公式(boatrace.jp) ─ scrape_motors.py ─→ docs/motor/motors_all.csv
Open-Meteo        ─ fetch_weather.py ─→ docs/data/weather.json
公式(mbrace LZH)  ─ scrapeKimarite.py ─→ docs/players/racerKimarite.csv（月2回）
        │
        ▼（出走表更新の完了をトリガに）
build_highlights.py
   ├─→ docs/highlights/highlights.json（公開・判定や主役艇は含めない）
   └─→ predictions/YYYYMMDD.json（非公開・検証用スナップショット）
build_arare.py ─→ docs/data/arare.json（荒れ指数の可視化）
```

### 2) 夜の結果照合（検証ループ）
```
公式(mbrace LZH) ─ buildResults.py ─→ results/YYYYMMDD.json（当日全24場の着順＋配当）
predictions/YYYYMMDD.json ＋ results/YYYYMMDD.json
        │ verifyPredictions.py（場×レースで突合）
        ▼
verify_log.csv（1レース1行で追記）
        │ build_verify_summary.py
        ▼
docs/data/verify_summary.json（検証の要約）
```
朝に出した見立て（`predictions/`）を、夜に確定した結果（`results/`）と突き合わせて
的中/傾向を記録する ＝ **見立ての精度を後から検証できる**設計。

### 3) 場別 払戻分析（徳山・桐生）
```
公式(mbrace LZH) ─ scrape*Payouts.py ─→ docs/payouts/*Payouts.csv（hd, rno, combo, payout）
        │ build*ManRate.py
        ▼
docs/payouts/*ManRate.json（R別の万舟率・平均配当・最高配当TOP5 など）
```

---

## 公開サイト（`docs/`, GitHub Pages）
- 入口: `docs/index.html`（データポータル）。主要カード: players / racers / motor / stadium。
- その他ページ: highlights（見どころ）, payouts（万舟率）, kiryu-payouts, uranai, announcers,
  stadium/gourmet, aisho-suminoe, shobuun-suminoe など。
- `docs/data/` の定数・ロジック: `stadium_orientation.json`（スタート→1M方位）,
  `tenkai_logic.json`（展開文の方針）, `tideTrend.json`, `wind_classify.js`, `arare.json`, `weather.json`。

---

## 開発メモ
- 24場コード（jcd）は各スクリプトにハードコード（01桐生 … 24大村）。
- タイムゾーンは JST（UTC+9）。Actions は UTC 実行なので JST 換算して対象日を決める
  （夜の実行は翌日分を予習対象にするロジックあり）。
- スクレイプ系は公式サーバ負荷軽減のため `SLEEP`（既定1.0秒）を挟む。
- 出力ファイルは基本 UTF-8。`build_highlights.py` の入力CSVは BOM 付き(`utf-8-sig`)を読む。
